...
static void *cli_rudp_handler ( void *arg) {
	...
	while (1) {
	 ...
	 time_out:	
		...
		sigprocmask(SIG_UNBLOCK, &timer_signal_set, NULL);
		printf("\n\n--------------------------------------------------------------\n");
		printf("Time: %.4f\trwnd in: bytes: %d\twin: %d\tseqNo: %d\tackNo: %d\n", delta_t, rwnd_byte, rwnd, pck_rcvd.seqNum, ackNo);			
		printf("+++Time: %.4f\treceive packet: ", delta_t);
		print_packet (pck_rcvd);
		if (pck_rcvd.type == DATA) {
			...
			/* update ackNo if receive expected seqNo, else resend ack to until receive expected packet */
			if (pck_rcvd.seqNum == ackNo + 1) {
				ackNo++;
				fputs(pck_rcvd.payload, dld_file_fp);
				printf("we put: %s into file\n", pck_rcvd.payload);
			}
			else
				printf("Discard received packet: %d\n", pck_rcvd.seqNum);
...
int main(int argc, char const *argv[])
{
	...
	cli_terminate (sockfd, (struct sockaddr *) &server_addr, addr_len);
